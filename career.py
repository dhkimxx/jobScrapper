...
def get_cnt_result(url):
    ...
    cnt_result = soup.find("em",{"class": "count"}).string
    result = re.findall("\d+",cnt_result) #숫자 추출 (module 're')
    cnt_result = int(result[0])
    return cnt_result

def extract_job(html):
    title = html.find("a",{"class":"summary"})
    if title:
        link = title["href"]
    ...
    condition = html.find("div",{"class":"t2"}).find("span").get_text(strip=True)
    return {
        'title':title,
        'company':company,
        'condition': condition,
...
def extract_jobs(url, last_page):
    ...
    for page in range(last_page):
        print(f"Scrapping career: page {page+1} / {last_page}")
...
def get_jobs(word):
    limit = 10  # 한 페이지에 출력되는 개수
    url = f"http://search.career.co.kr/pc/Jobs?kw={word}"
